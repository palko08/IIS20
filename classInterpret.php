...
class Interpret{
    ...
    function getCas_vystupenia($pdo, $podium_ID){
        ...
        $select->execute(array($podium_ID, $this->interpretID));
        return $select->fetchColumn();
    }
    ...
    function checkZaner($pdo, $zaner_ID){
        ...
        $select->execute(array($zaner_ID, $this->interpretID));
        if($select->rowCount() == 0){
            return 1;
        }else{
            return 0;
    ...
    function checkClen($pdo, $clen_ID){
        ...
        $select->execute(array($clen_ID, $this->interpretID));
        if($select->rowCount() == 0){
            return 1;
        }else{
            return 0;
    ...
    function checkVystupenie($pdo, $podium_ID){
        ...
        $select->execute(array($this->interpretID, $podium_ID));
        if($select->rowCount() == 0){
            return 1;
        }else{
            return 0;
    ...
    function setNazov($pdo, $data){
    	try{
    		...
    		$select->execute(array($data, $this->interpretID));
    		return 0;
    ...
    function addZaner($pdo, $zaner_ID){
        ...
        $insert->execute(array($this->interpretID, $zaner_ID));
        return 0;
    }
    ...
    function deleteZaner($pdo, $zaner_ID){
        ...
        $delete->execute(array($zaner_ID, $this->interpretID));
    ...
    function addClen($pdo, $clen_ID){
        ...
        $insert->execute(array($this->interpretID, $clen_ID));
        return 0;
    }
    ...
    function deleteClen($pdo, $clen_ID){
        ...
        $delete->execute(array($clen_ID, $this->interpretID));
    ...
    function addVystupenie($pdo, $podium_ID, $cas_vystupenia){
        ...
        $insert->execute(array($this->interpretID, $podium_ID, $cas_vystupenia));
        return 0;
    }
    ...
    function deleteVystupenie($pdo, $interpret_ID){
        ...
        $delete->execute(array($podium_ID, $this->interpretID));
